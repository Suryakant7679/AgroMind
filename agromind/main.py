...
DEFAULT_AI_TUTOR_URL = "http://127.0.0.1:8010"

# Create local upload storage; retain fallback handling for unwritable directories.
...
def chatbot_page(request: Request):
    ...
    if isinstance(user_or_response, RedirectResponse):
        return user_or_response
    ai_tutor_url = (os.getenv("AI_TUTOR_URL", "").strip() or DEFAULT_AI_TUTOR_URL).rstrip("/")
    return page(
        request,
        "chatbot.html",
