...
def test_chatbot_never_falls_back_to_workspace_ui(monkeypatch):
    monkeypatch.setattr(main, "user_from_session", lambda request: {"id": "user-1", "email": "u@example.com"})
    monkeypatch.setenv("AI_TUTOR_URL", "")
    with TestClient(main.app) as client:
        response = client.get("/chatbot")
    assert response.status_code == 200
    assert 'src="http://127.0.0.1:8010"' in response.text
    assert "sendChatbotMessage" not in response.text
    assert "Workspace Chatbot" not in response.text


def test_chatbot_respects_configured_tutor(monkeypatch):
    monkeypatch.setattr(main, "user_from_session", lambda request: {"id": "user-1", "email": "u@example.com"})
    monkeypatch.setenv("AI_TUTOR_URL", "http://127.0.0.1:8020/")
    with TestClient(main.app) as client:
        response = client.get("/chatbot")
    assert 'src="http://127.0.0.1:8020"' in response.text
